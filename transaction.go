...
func (t *CSVHunk) TransactionsFromCSV() ([]Transaction, error) {
	records, err := t.CSVRecords()
	if err != nil {
		return []Transaction{}, err
	}
	transactions := make([]Transaction, len(records))
	for i, dfr := range records {
		transactions[i] = *transactionFromRecord(dfr, t.columnMapper)
	}
	return transactions, nil
}

func (t *CSVHunk) CSVRecords() ([][]string, error) {
	if t.df.Err != nil {
...
func transactionFromRecord(record []string, m map[string]int) *Transaction {
